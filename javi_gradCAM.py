...
def mainSimpleImage(args):
    ...
    if args.layer == 'all':
        ...
        image = preProcessImage(path)   
        locMap, _, _ = gradCAM.getLocalizationMap(image, c)
        gradCAMpp = GradCAMPlusPlus(model, args.layer)
        ...
        if not args.resultsPath == 'None':
            ...
            plt.imsave(args.resultsPath + args.layer + '_++' + str(c) + name, np.uint8(gradCAMpp.getHeatmap(locMappp, image)))
